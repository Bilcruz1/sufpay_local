...
import {
	...
	IValidateToken,
	IVerifyAccount,
	IVerifyEmailUniqueness,
	IVerifyPhoneNumberUniqueness,
...
export const register = async (data: ISigupForm): Promise<IResponse> => {
	try {
		const res = await http.post('/Authentication/register', data);
...
export function signupRequest(data: ISigupForm) {
	return handleRequest(() =>
		clientRequest({ baseURL: BASE_URL }).post('/Authentication/register', data)
	);
}
...
export function loginRequest(data: ILogin) {
	return handleRequest(() =>
		clientRequest({ baseURL: BASE_URL }).post('/Authentication/login', data)
	);
}
