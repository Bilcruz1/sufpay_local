...
import { DashboardCardsContent, transactionsTile } from "../../utils/constants";
import DashboardCards from "./DashboardCards";
import DashboardHistories from "./DashboardHistories";
...
const DashboardLanding = () => {
  return (
    <Box>
      ...
      {/* body */}
      <Grid container spacing={"1rem"} sx={{ marginTop: ".5rem", minHeight: "70vh" }}>
        {/* RHS */}
        <Grid item xs={12} md={8} sx={{ }}>
          <Stack direction={"column"} spacing={"1rem"}>
              {transactionsTile.map((el, ind) => (
                <Box flex={1} sx={{heigth: "35vh"}}>
                    <DashboardHistories key={ind} title={el.title} linkUrl={el.linkUrl} />
                </Box>
              ))}
          </Stack>
        </Grid>
